...
def buscar():
    ...
    if _identificacion=='':
        flash('Recuerde ingresar el numero de identifcación.')
    else:
        sql="SELECT aut.Nombre, pr.Nombre_Proyecto, pr.Modalidad FROM proyecto pr, persona aut WHERE aut.Identificacion=%s AND pr.Autor = aut.Id"
        ...
        if datosRecibidos ==1:
            ...
            #text width
            text_widthNombre, _=fontNombre.getsize(fila[0][0])
        
            draw = ImageDraw.Draw(image)
            #escribe el NOMBRE en la imagen
            draw.text(((image_width - text_widthNombre) / 2,text_y_position_Nombre), fila[0][0], font=fontNombre, fill="red")
            #escribe el PROYECTO en la imagen
            draw.text((360,432), fila[0][1], font=fontProyecto, fill="red")
            #escribe la MODALIDAD en la imagen
            draw.text((730,462), fila[0][2], font=fontProyecto, fill="red")
